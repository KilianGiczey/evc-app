...
interface CapitalCostsSectionProps {
  projectId: string;
}

export default function CapitalCostsSection({ projectId }: CapitalCostsSectionProps) {
  ...
  const loadCosts = async () => {
    ...
    setError(null);
    try {
      const data = await getCostsByType(projectId, 'Capex');
      setCosts(data || []);
    } catch (err) {
      setError('Failed to load capital costs');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="pt-2 mb-4">
      <div className="flex items-center justify-between mb-4">
        <h4 className="text-md font-semibold text-gray-900 dark:text-white">Capital Costs</h4>
        <button
